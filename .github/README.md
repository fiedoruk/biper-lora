...
| **A panel served from the cube's own flash** | Join the cube's Wi-Fi and the phone becomes a screen and a keyboard. No account, no app store, no internet. 122 kB of HTML, 46.3 kB (47451 bytes) over the air after gzip. It carries a built-in guide, so the manual is inside the device. |
...
```
73 files changed, 15584 insertions(+), 373 deletions(-)   # vs MeshCore companion-v1.17.1, as of 5 Sep 2026
```
